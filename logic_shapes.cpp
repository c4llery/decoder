...
ConnectionShape::ConnectionShape (const ElementShape& elem, const OperatorShape& oper, size_t i)
    : line1 { elem.output_pos(), {elem.output_pos().x, oper.input_pos(elem.parent(), i).y} }
    , line2{ {elem.output_pos().x, oper.input_pos(elem.parent(), i).y}, oper.input_pos(elem.parent(), i)}
  ...
  , circle { oper.input_pos(elem.parent(), i), CIRCLES_RADIUS }
{
  ...
  // set styles
  line1.set_style(CONNECTION_LINE_TYPE);
  line2.set_style(CONNECTION_LINE_TYPE);

  dash.set_style(CONNECTION_LINE_TYPE);
  circle.set_style(CIRCLE_LINE_TYPE);

  line1.set_color(elem.parent() ? TRUE_COLOR : FALSE_COLOR);
  line2.set_color(elem.parent() ? TRUE_COLOR : FALSE_COLOR);
